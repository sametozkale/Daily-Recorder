...
import { getMediaPhotos } from "./provider-card-stack"
...
export function LifelinePhotoCard({
  ...
  const onPointerUp = (event: PointerEvent<HTMLDivElement>) => {
    ...
    // (or open GitHub for branded logo cards).
    if (!drag.current.moved && !lightboxStart) {
      const isGitHubBrand =
        photo.provider === "github" &&
        (!photo.src || photo.src.includes("/brands/github"))
      if (isGitHubBrand && photo.href) {
  ...
  return (
    <>
      ...
      >
        <div
          className={cn(
            "relative overflow-hidden rounded-xl shadow-xl ring-1 ring-black/10 transition-[transform,box-shadow] duration-200 ease-out dark:ring-white/15",
        ...
        >
          {photo.provider === "github" &&
          (!photo.src || photo.src.includes("/brands/github")) ? (
            <div className="pointer-events-none flex aspect-[4/3] w-full flex-col items-center justify-center gap-3 bg-zinc-100 px-4 dark:bg-zinc-900">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src="/brands/github.svg"
                alt=""
                className="size-10 opacity-90 dark:invert"
              />
              <p className="line-clamp-2 text-center text-xs font-medium text-zinc-600 dark:text-zinc-300">
                {photo.alt}
              </p>
            </div>
          ) : (
          ...
          {photo.provider ? (
            ...
            <img
              ...
              alt={photo.provider === "github" ? "GitHub" : "Figma"}
              className={cn(
                "pointer-events-none absolute left-2 top-2 size-5 rounded-md bg-white/90 p-0.5 shadow-sm ring-1 ring-black/10 dark:bg-black/80 dark:ring-white/15",
  ...
  return (
    <div aria-hidden="true" className="pointer-events-none absolute inset-0">
      {markers.map((marker, index) => {
        // GitHub/Figma logo cards live in the marker column stack.
        const photos = getMediaPhotos(marker.photos)
        ...
        const lastPhoto = photos[photoCount - 1]
        const groupWidth =
